...
"""
Node for driving in donuts with car using pure pursuit controller.
"""
...
pure_pursuit.Lfc = 0.7  # look-ahead distance
...
def smooth_donut_path(c, r, phase=0):
    ...
    cx, cy, cyaw, ck, s = cubic_spline_planner.calc_spline_course(xs, ys, ds=0.1)
...
def main():
    ...
    while laps < target_laps and not rospy.is_shutdown():
        cx, cy, cyaw = smooth_donut_path(geofence.center, path_radius, laps*math.pi/3)
        lastIndex = len(cx) - 1
        target_ind = 0
